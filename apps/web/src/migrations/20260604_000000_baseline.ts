...
// Squashed schema baseline (production shape as of 2026-06-04, including
// Lexical body fields). Every statement is a static tagged-template literal:
// no query text is built by concatenation or interpolation at runtime.
// - Fresh DB: run via `payload migrate:fresh` (drops public schema, then applies this).
// - Existing prod/local: do not re-run `up`; reset `payload_migrations` only
//   (see scripts/reset-payload-migrations-to-baseline.sql).
const baselineStatements = [
  ...
  sql`CREATE TABLE public._active_memberships_v (
    id integer NOT NULL,
    parent_id integer,
  ...
  sql`CREATE SEQUENCE public._active_memberships_v_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1`,
  sql`ALTER SEQUENCE public._active_memberships_v_id_seq OWNED BY public._active_memberships_v.id`,
  ...
  sql`CREATE TABLE public.active_memberships (
    id integer NOT NULL,
  ...
  sql`CREATE SEQUENCE public.active_memberships_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1`,
  sql`ALTER SEQUENCE public.active_memberships_id_seq OWNED BY public.active_memberships.id`,
  ...
  sql`ALTER TABLE ONLY public._active_memberships_v ALTER COLUMN id SET DEFAULT nextval('public._active_memberships_v_id_seq'::regclass)`,
  ...
  sql`ALTER TABLE ONLY public.active_memberships ALTER COLUMN id SET DEFAULT nextval('public.active_memberships_id_seq'::regclass)`,
  ...
  sql`ALTER TABLE ONLY public.active_memberships
  ...
  sql`CREATE INDEX _active_memberships_v_created_at_idx ON public._active_memberships_v USING btree (created_at)`,
  sql`CREATE INDEX _active_memberships_v_parent_idx ON public._active_memberships_v USING btree (parent_id)`,
  sql`CREATE INDEX _active_memberships_v_updated_at_idx ON public._active_memberships_v USING btree (updated_at)`,
  sql`CREATE INDEX _active_memberships_v_version_version_created_at_idx ON public._active_memberships_v USING btree (version_created_at)`,
  sql`CREATE INDEX _active_memberships_v_version_version_normalized_rut_idx ON public._active_memberships_v USING btree (version_normalized_rut)`,
  sql`CREATE INDEX _active_memberships_v_version_version_rut_lookup_hash_idx ON public._active_memberships_v USING btree (version_rut_lookup_hash)`,
  sql`CREATE INDEX _active_memberships_v_version_version_updated_at_idx ON public._active_memberships_v USING btree (version_updated_at)`,
  ...
  sql`CREATE INDEX active_memberships_created_at_idx ON public.active_memberships USING btree (created_at)`,
  sql`CREATE UNIQUE INDEX active_memberships_normalized_rut_idx ON public.active_memberships USING btree (normalized_rut)`,
  sql`CREATE UNIQUE INDEX active_memberships_rut_lookup_hash_idx ON public.active_memberships USING btree (rut_lookup_hash)`,
  sql`CREATE INDEX active_memberships_updated_at_idx ON public.active_memberships USING btree (updated_at)`,
  ...
  sql`CREATE INDEX payload_locked_documents_rels_active_memberships_id_idx ON public.payload_locked_documents_rels USING btree (active_memberships_id)`,
